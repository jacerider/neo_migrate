...
Props take `{from: <field>, transform: …}` or a fixed `{value: …}`. Transforms: `string`; `markup` (rewritten by the `markup` rules); `flag` (TRUE when the value equals `when`, or is truthy); `link` (uri, title, options); `number` (a value from `key`, divided by `divide` and rounded to `precision` decimals — a 0–100 rating into 0–5 stars, a coordinate at `precision: 6`); `wrap` (a plain value as rich text in one tag: `{transform: wrap, from: field_title, tag: h2}`); `each` with `from: '@self'` (a legacy item that stood alone where the component expects a list, as a list of one); `target` (an entity reference's target id, for a filter); `heading` (one field per part: `{transform: heading, supertitle: field_a, title: field_b}`); `each` (nested items as an array prop, one entry per published item, each from its own `props:`, with the same filled-field rule); `image_media` (the file as an image media entity, reusing one that holds the same file with the same alt text; the bundle and source field default to `image` / `field_media_image`, or set `media: {image: {bundle, field}}`; with `as: <key>`, every value of a multi-value field as array entries `{<key>: …}`). A fixed `{value: lg}` is wrapped as a field item.

`filters:` sets a component filter per instance, named by the filter's title — how an embedded webform is picked per page: the component has an editable Webform entity filter feeding an Entity slot, and the mapping gives `filters: {Webform: {from: field_form, transform: target}}`. Read-back checks the filter's value.
